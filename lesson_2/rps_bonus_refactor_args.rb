...
module Displayable
  ...
  def display_winner(human, computer, log)
    puts format(preamble_text(log), game_winner_data)
...
class RPSGame
  ...
  def play_again?
    loop do
      ...
      case answer
      when TEXT['history'] then display_history(log)
  ...
  def play_all_rounds
    ...
    display_winner(human, computer, log)
  end
  ...
  def setup
    ...
    introduce_opponent
    rules_check
    self.winning_points = prompt_for_winning_points
  end
